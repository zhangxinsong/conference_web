...
export default {
  ...
  methods: {
    insertScriptTag () {
      ...
      let uploadConfigScriptTag = document.getElementById('uploadConfigScriptTag');

      // 如果这个tag不存在，则生成相关代码tag以加载代码
      if (editorScriptTag === null) {
        configScriptTag = document.createElement('script');
        configScriptTag.src = this.ueditorPath + 'ueditor.config.js?v=' + this.timestamp; configScriptTag.id = 'configScriptTag';

        editorScriptTag = document.createElement('script');
        editorScriptTag.src = this.ueditorPath + 'ueditor.all.js?v=' + this.timestamp; editorScriptTag.id = 'editorScriptTag';

        uploadConfigScriptTag = document.createElement('script');
        uploadConfigScriptTag.src = this.ueditorPath + 'upload.config.js?v=' + this.timestamp; uploadConfigScriptTag.id = 'uploadConfigScriptTag';

        let s = document.getElementsByTagName('head')[0];
        s.appendChild(configScriptTag);
        s.appendChild(editorScriptTag);
        s.appendChild(uploadConfigScriptTag);
      }
